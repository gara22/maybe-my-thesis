...
import { Booking, BookingWithAllData, BookingWithBooker, Classroom } from "../types/types";
...
const getBookings = async (from: Date, to: Date, id: string): Promise<{ bookings: BookingWithBooker[] }> => {
...
const createClassroom = (data: Pick<Classroom, 'name' | 'capacity' | 'hasComputer'>) => {
...
const getFreeClassrooms = async (from: Date, to: Date, hasComputer: boolean): Promise<{ freeClassrooms: Classroom[] }> => {
