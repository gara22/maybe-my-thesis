...
const ClassroomShow = () => {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useQuery(['classroom', id], () => API.classrooms.getClassroomById(id as string), { refetchOnWindowFocus: false });
  const {
    data: bookingsData,
  ...
  const { mutate: createBooking, isLoading: isCreateLoading } = useMutate<BookingParams>(
    API.bookings.createBooking,
    { onSuccess: refetchBookings, onSettled: onCloseCreate },
    {
  ...
  const { mutate: editBooking, isLoading: editLoading } = useMutate<Pick<Booking, 'id' | 'description'>>(
    API.bookings.editBooking,
  ...
  if (!classroom) return <div>no classroom</div>;
        ...
        >
          <BookingForm
            ...
            defaultValues={{
              ...
              date: selectedDate,
              description: bookings?.find((b) => b.id === selectedBookingId)?.description || '',
            }}
            isEdit
            isLoading={editLoading || isDeleteLoading}
          />
...
const WeekSelector = ({
  ...
  startDate: Dayjs;
  onDateChange: (updatedDate: Dayjs) => void;
}) => {
  return (
    <Flex justifyContent="center" padding="10" fontSize="3xl" alignItems="center">
      ...
      Current week is: {startDate.format('YYYY/MM/DD')} -{' '}
      {dayjs(startDate)
        .add(LENGTH_OF_WEEK - 1, 'day')
        .format('YYYY/MM/DD')}
