...
export const Home = () => {
  ...
  const {
    ...
    refetch,
    isFetching,
  } = useQuery(['freeClassrooms', inputs.from, inputs.to, inputs.hasComputer], getFreeClassrooms, {
            ...
            classrooms.map((r) => (
              ...
              <Card bg="gray.600" key={r.id}>
                <CardBody>
                  ...
                  <Text fontSize="xs">
                    {' '}
                    {dayjs(inputs.from).format('YYYY/MM/DD HH:00')} - {dayjs(inputs.to).format('HH:00')}
                  </Text>
