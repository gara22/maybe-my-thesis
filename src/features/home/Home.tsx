...
export const Home = () => {
  const [inputs, setInputs] = useState<SearchParams & { enabled: boolean }>({
    to: new Date(),
    from: new Date(),
    hasComputer: false,
    enabled: false,
  ...
  const getFreeClassrooms = async (): Promise<{
  ...
  const {
    ...
    ["freeClassrooms", inputs.from, inputs.to, inputs.hasComputer],
    getFreeClassrooms,
    {
            ...
            classrooms.map((r) => (
              ...
              <Card bg="gray.600" key={r.id}>
                <CardBody>
                  ...
                  <Text fontSize="xs">
                    {" "}
                    {dayjs(inputs.from).format("YYYY/MM/DD HH:00")} -{" "}
                    {dayjs(inputs.to).format("HH:00")}
                  </Text>
                  <UnorderedList
                    fontSize="sm"
                    paddingLeft="10px"
                  ...
                  >
                    ...
                    <ListItem>
                      Has computers: {r.hasComputer ? "Yes" : "No"}
                    </ListItem>
                  </UnorderedList>
                </CardBody>
                <Button>
